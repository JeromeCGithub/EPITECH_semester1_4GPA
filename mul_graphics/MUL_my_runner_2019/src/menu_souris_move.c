...
void manage_menu_play_inf(sfRenderWindow *window, sfSprite **button)
{
    ...
    if (pos_souris.x >= sprite.x + int_cv(window, 65, 0)
        ...
        sfSprite_setScale(button[INF_BUTTON], scale);
        sfSprite_setPosition
            (button[INF_BUTTON], convert_vector2f(window, 630, 580));
    }
    else {
        sfSprite_setScale(button[INF_BUTTON], scale_origin);
        sfSprite_setPosition
            (button[INF_BUTTON], convert_vector2f(window, 650, 600));
    }
}

void manage_menu_mouse_quit(sfRenderWindow *window, sfSprite **button)
{
    sfVector2f scale_origin = convert_vector2f(window, 0.2, 0.2);
    sfVector2f scale = convert_vector2f(window, 0.22, 0.22);
    ...
    manage_menu_play_inf(window, button);
}
...
void manage_menu_mouse_play(sfRenderWindow *window, sfSprite **button)
{
    ...
    manage_menu_mouse_quit(window, button);
}
