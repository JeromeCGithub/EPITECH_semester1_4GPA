...
static char *fill_output(int *j, char const token,
                        char *output, char *op_stack)
{
...
{
    int j = 0;
    char *output = malloc((len_output(str)) * sizeof(char));
    char *op_stack = malloc((len_op_stack(str)) * sizeof(char));

    test_malloc(output);
    test_malloc(op_stack);
    output = my_memset(output, len_output(str), '\0');
    op_stack = my_memset(op_stack, len_op_stack(str), '\0');
